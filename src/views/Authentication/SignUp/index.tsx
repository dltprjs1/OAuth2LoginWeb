...
import { CheckCertificationResponseDto, EmailCertificationResponseDto, SignUpresponseDto } from 'apis/response/auth';
...
export default function SignUp() {
  ...
  const emailCertificationResponse = (responseBody: ResponseBody<EmailCertificationResponseDto>) => {
    ...
    if (code === ResponseCode.MAIL_FAIL) alert('아이디와 이메일을 모두 입력하세요.');
    if (code === ResponseCode.DATABASE_ERROR) alert('데이터베이스 오류 입니다.');
    if (code !== ResponseCode.SUCCESS) return;
    setEmailError(false);
  ...
  const onSignUpButtonClickHandler = () => {
    ...
    if (!id || !email || !password || !passwordCheck ||!certificationNumber) return;
    if (!isIdCheck) {
      alert('중복 확인은 필수입니다.');
      return;
    };
    ...
    <div id='sign-up-wrapper'>
        ...
        <div className='sign-up-container'>
            <div className='sign-up-box'>
                <div className='sign-up-title'>{'자동 선물 프로그램'}</div>
                <div className='sign-up-content-box'>
                    ...
                    <div className='sign-up-content-button-box'>
                        <div className={`${signupButtonClass} full-with`} onClick={onSignUpButtonClickHandler}>{'회원가입'}</div>
